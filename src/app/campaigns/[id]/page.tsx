  ...
  return (
    <div className="mx-auto max-w-7xl px-6 py-8">
        ...
        {total > 0 && (
          <div className="mt-3 flex flex-wrap gap-2">
            {Object.entries(counts).map(([status, count]) => (
              <span
                key={status}
                className={`rounded-full px-2.5 py-0.5 text-sm font-medium ${STATUS_COLORS[status] ?? STATUS_COLORS.new}`}
              >
                {count} {status}
              </span>
            ))}
            <span className="rounded-full px-2.5 py-0.5 text-sm font-medium bg-zinc-900 text-white dark:bg-zinc-100 dark:text-zinc-900">
              {total} total
            </span>
          </div>
        )}
      ...
      <section className="mb-6 rounded-lg border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-950">
        ...
        <div className="flex flex-wrap gap-2">
          <form action={runDiscovery.bind(null, campaign.id)}>
            <button className="rounded bg-zinc-900 px-3 py-1.5 text-sm font-medium text-white hover:bg-zinc-700 dark:bg-zinc-100 dark:text-zinc-900 dark:hover:bg-zinc-300">
              1. Discover
            </button>
          </form>
          ...
          <form action={detectRepliesAction.bind(null, campaign.id)}>
            <button className="rounded border border-zinc-300 px-3 py-1.5 text-sm font-medium text-zinc-600 hover:bg-zinc-50 dark:border-zinc-700 dark:text-zinc-400 dark:hover:bg-zinc-900">
              Detect replies
            </button>
          </form>
        ...
        <details className="rounded-lg border border-zinc-200 bg-white p-4 text-sm dark:border-zinc-800 dark:bg-zinc-950">
          ...
          >
            <textarea
              ...
              className="w-full rounded border border-zinc-300 px-2 py-1.5 text-sm font-mono dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-200"
            />
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 rounded border border-zinc-300 px-2 py-1.5 text-sm dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-200"
              />
              <input
                name="maxResults"
                type="number"
                ...
                min={5}
                max={50}
                className="w-16 rounded border border-zinc-300 px-2 py-1.5 text-sm dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-200"
              />
            </div>
            <button className="rounded bg-zinc-900 px-3 py-1.5 text-sm font-medium text-white hover:bg-zinc-700 dark:bg-zinc-100 dark:text-zinc-900 dark:hover:bg-zinc-300">
              Scrape
      ...
      <details className="mb-6 text-sm text-zinc-500 dark:text-zinc-400">
        ...
        <pre className="mt-2 whitespace-pre-wrap rounded-lg bg-zinc-50 p-4 text-sm dark:bg-zinc-900 dark:text-zinc-300">
          {campaign.pitch}
        </pre>
      </details>
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span className={`rounded-full px-2 py-0.5 text-xs uppercase font-medium ${statusColor}`}>
                      {lead.status}
                    </span>
                    {lead.fitScore != null && (
                  ...
                  <div className="mt-1 flex flex-wrap gap-x-2 text-sm text-zinc-500 dark:text-zinc-400">
                    {lead.website && (
                  ...
                  {enriched?.signals?.length > 0 && (
                    <div className="mt-1 flex flex-wrap gap-1">
                      {enriched.signals.map((s: string) => (
                        <span key={s} className="rounded bg-zinc-100 px-1.5 py-0.5 text-xs text-zinc-500 dark:bg-zinc-800 dark:text-zinc-400">
                          {s}
                        </span>
                      ))}
                    </div>
                ...
                <div className="flex shrink-0 gap-1.5">
                  {draft && lead.status !== "handed_off" && (
                    <form action={approveAndPushDraft.bind(null, draft.id)}>
                      <button className="rounded bg-emerald-600 px-2.5 py-1 text-sm font-medium text-white hover:bg-emerald-700 dark:bg-emerald-500 dark:hover:bg-emerald-400">
                        Send
                      </button>
                    </form>
                  )}
                  {lead.status === "drafted" && (
                    <form action={regenerateDraftForLead.bind(null, lead.id)}>
                      <button className="rounded border border-zinc-300 px-2.5 py-1 text-sm text-zinc-500 hover:bg-zinc-50 dark:border-zinc-700 dark:text-zinc-400 dark:hover:bg-zinc-900">
                        Regen
                      </button>
                    </form>
                  )}
                  {lead.status === "handed_off" && !lead.repliedAt && (
                    <form action={markRepliedAction.bind(null, lead.id)}>
                      <button className="rounded border border-violet-300 px-2.5 py-1 text-sm text-violet-600 hover:bg-violet-50 dark:border-violet-700 dark:text-violet-400 dark:hover:bg-violet-950">
                        Replied
                      </button>
                    </form>
                  )}
                  {lead.status !== "rejected" && lead.status !== "replied" && (
                    <form action={rejectLead.bind(null, lead.id)}>
                      <button className="rounded border border-zinc-200 px-2.5 py-1 text-sm text-zinc-400 hover:bg-zinc-50 dark:border-zinc-800 dark:text-zinc-500 dark:hover:bg-zinc-900">
                        ✕
                      </button>
                    </form>
                  )}
              ...
              {draft && (
                ...
                >
                  ...
                  <textarea
                    ...
                    rows={5}
                    className="w-full rounded border border-zinc-200 px-2 py-1.5 font-mono text-sm leading-relaxed dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-300"
                  />
                  <button className="rounded border border-zinc-300 px-2.5 py-1 text-sm text-zinc-500 hover:bg-zinc-50 dark:border-zinc-700 dark:text-zinc-400 dark:hover:bg-zinc-900">
                    Save
                  </button>
